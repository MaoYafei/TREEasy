...
###alignment sequence###
        ...
        os.system("mafft --localpair --maxiterate 1000 %s_aa.fasta > %s_aa.aln 2> %s_mafft_aln.log" % (pwd+seq_file,pwd+seq_file,pwd+seq_file))
	os.system("perl translatorx_vLocal.pl -i %s.del -a %s_aa.aln -o %s 2> tranlatorx.log" %(pwd+seq_file,pwd+seq_file,pwd+seq_file))
...
####running iqtree###
	if len_nc_aln!=0:
		...
		outfile.write("#nexus\nbegin sets;\n\tcharset p1 = 1-%d \ 3;\n\tcharset p2 = 2-%d \ 3;\n\tcharset p3 = 3-%d \ 3;\nend;" % (len_nc_aln,len_nc_aln,len_nc_aln))
		outfile.close()
		os.system("iqtree -s %s -bb 1000 -redo -nt %d -spp %s.nex -m MFPMERGE 1>%s_ML_iqtree.log" %(pwd+seq_file+'.nt.aln',thread_number_iqtree,pwd+seq_file+".nt.aln",pwd+seq_file))	
...
def aln_noncds(Parameters):
	...
	thread_number_iqtree=Parameters[1]
        os.system("mafft --localpair --maxiterate 1000 %s.fasta > %s.aln 2> %s_mafft_aln.log" % (pwd+seq_file,pwd+seq_file,pwd+seq_file))
####running iqtree###
        os.system('mv %s.aln %saln_seqs/' % (pwd+seq_file,pwd))
        pwd=pwd+'/aln_seqs/'
        os.system("iqtree -s %s -bb 1000 -redo -nt %d -m MFP 1>%s_ML_iqtree.log" %(pwd+seq_file+'.aln',thread_number_iqtree,pwd+seq_file)) 
